...
y <- 3 * x[, 1] + x[, 2] + error
```
The `cqs` function proceeds by specifying an $n \times p$ design matrix $\mathbf{X}$, a vector of the response variable $Y$, and a quantile level $\tau$.  The rest of the arguments, i.e., $d$, the dimension of the central subspace spanned by $\mathbf{A}$, and $d_{\tau}$, the dimension of the $\tau$-CQS, are not rquired. Note that for this example, $d=d_{\tau}=1$ for every quantile level $\tau$  
...
```
If we want to measure the estimation error resulting from estimating the $\tau$-CQS, we can use the angle between the true and the estimated subspace.  This is performed using the `subspace` function of the `pracma` package.  Note that the angle is measured in radians, and so we can dividy by $\pi / 2$.
``` {r}
library(pracma)
...
```
Note that the estimation error is slightly higher when the dimension of the subspace is unknown and being esitmated. 
...
qhat2
```
If $h$ is specified, then the output reports the value inputed by the user, i.e.,
``` {r}
