...
Value-at-risk (VaR) is an important financial quantity directly related to the quantile regression framework.  Specifically, for $\{r_{t}\}_{t=1}^{n}$ a time series of returns, the $\tau$th VaR at time $t$, denoted by $VaR_{\tau}(t)$, is the smallest number for which $P\{r_{t} < -VaR_{\tau}(t) | \mathcal{F}_{t-1}\} = \tau$, where $\mathcal{F}_{t-1}$ denotes the information set at time $t-1$.  Therefore, $-VaR_{\tau}(t)$ is the $\tau$th conditional quantile of $r_{t}$, and we can write $Q_{\tau}(r_{t} | \mathcal{F}_{t-1}) = -VaR_{\tau}(t)$.   
...
head(edhec)
```
To estimate the VaR of the next day we need a vector of returns.  For the purpose of this, we will select the 'Convertible Arbitrage' as a vector of daily returns.  The `ValAR` function proceeds by specifying $p$, the number of past observations to be used as the predictor variables, the quantile level $\tau$, and a moving window for the number of observations to be used for fitting the model.  If a moving window is not specified, then all the observations will be used to estimate the conditional quantile.  Note that, typical values for moving windows are 250 and 500, corresponding to one or two years of daily returns.  A moving window will be useful if the number of observations is large and returns from way in the past will not be of any value to the prediction.  Moreover, if the number of observations is large, a moving window will also speed up the algorithm, since $n$ will be smaller.  Finally, if the returns are in reversed chronological order, then the user should either reverse the vector of daily returns or indicate that using the `chronological` option of the function. 

For this example, we will use $p = 5$, $\tau = 0.05$, and will not specify any moving window since the number of observations is 275.  Note that, the last day of returns is ... and therefore, we are predicting the 5\% VaR for ....    
