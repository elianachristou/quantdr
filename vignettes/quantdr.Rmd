...
head(edhec)
```
To estimate the one-step ahead VaR we need a vector of returns in a standard chronological order.  For this, we will select the 'Convertible Arbitrage' as a vector of daily returns.  The `ValAR` function proceeds by specifying $p$, the number of past observations to be used as the predictor variables, the quantile level $\tau$, and a moving window for the number of observations to be used for fitting the model.  If a moving window is not specified, then all the observations will be used to estimate the conditional quantile.  Note that, typical values for moving windows are 250 and 500, corresponding to one or two years of daily returns.  A moving window will be useful if the number of observations is large and past returns will not be of any value to the prediction.  Moreover, if the number of observations is large, a moving window will also speed up the algorithm, since the number of used observations will be smaller.  Finally, if the returns are in reversed chronological order, then the user should either reverse the vector of daily returns or indicate that using the `chronological` option of the function. 

For this example, we will use $p = 5$, $\tau = 0.05$, and will not specify any moving window since the number of observations is 275.  Note that, the last day of returns is 2019-11-30 and therefore, we are predicting the 5\% VaR for 2019-12-31.     
...
```
To visualize the results, we can ...
```{r}
...
```
which is really close to 0.05. 

We can repeat that for $\tau= 0.01, 0.025$ and add the lines.
```{r}
